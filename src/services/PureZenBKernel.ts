...
/**
*    ZENB KERNEL V6.0 - NEURO-SOMATIC READY
* ========================================
*/
...
function reduce(state: RuntimeState, event: KernelEvent): RuntimeState {
  switch (event.type) {
    ...
    case 'AI_INTERVENTION':
        // Handle Gemini Tool Calls here. 
        // For now, mostly logs, but could dynamically swap patterns.
        return { ...state, lastUpdateTimestamp: event.timestamp, aiActive: true };

    case 'AI_VOICE_MESSAGE':
        return { ...state, lastAiMessage: event.text, lastUpdateTimestamp: event.timestamp };
...
export class PureZenBKernel {
  ...
  // Performance throttling for UI updates
  private lastNotifyTime = 0;
  private readonly NOTIFY_INTERVAL = 16; // ~60fps max updates to React
  ...
  private notify(): void {
      ...
      // Always notify for critical state changes immediately
      const isCritical = this.state.status === 'SAFETY_LOCK' || 
                         this.state.phase !== this.lastNotifiedPhase ||
                         this.state.status !== 'RUNNING' ||
                         this.state.lastAiMessage !== null; // Notify on AI message
