...
import { X, Volume2, VolumeX, Smartphone, SmartphoneNfc, Music, Check, Terminal, ShieldAlert, Sparkles } from 'lucide-react';
...
export function SettingsSheet() {
  ...
  const handleCameraToggle = () => {
      ...
      if (!userSettings.cameraVitalsEnabled) {
          // Turning ON: Show permission explainer first
  ...
  const handleResetLocks = () => {
      triggerHaptic();
      // Reset locks for all known patterns to be safe
      ['4-7-8', 'box', 'calm', 'coherence'].forEach(id => {
         resetSafetyLock(id as BreathingType);
      });
                  ...
                  <div className="space-y-3">
                      ...
                      <label className="flex items-center justify-between p-5 bg-white/[0.02] rounded-[1.5rem] border border-white/5 cursor-pointer hover:bg-white/[0.04] transition-colors">
                        <div>
                          ...
                          <div className="text-xs text-white/40 mt-1">Adaptive Control</div>
                        </div>
                  ...
                  <div className="space-y-3">
                     <label className="flex items-center justify-between p-5 bg-gradient-to-r from-purple-500/10 to-blue-500/10 rounded-[1.5rem] border border-white/10 cursor-pointer hover:border-white/20 transition-colors">
                        ...
                        <div className={clsx("w-11 h-6 rounded-full relative transition-colors border border-white/10", userSettings.aiCoachEnabled ? "bg-purple-500" : "bg-white/10")}>
                          <input 
                            type="checkbox" 
                            checked={userSettings.aiCoachEnabled} 
                            onChange={() => { triggerHaptic(); toggleAiCoach(); }}
                            className="sr-only"
                          />
                    ...
                    <button onClick={() => { triggerHaptic(); toggleKernelMonitor(); setSettingsOpen(false); }} className="w-full p-4 bg-white/5 rounded-xl flex items-center gap-3 text-white/60 hover:text-white hover:bg-white/10 transition-colors">
                        <Terminal size={16} />
                        <span className="text-xs font-mono">Kernel Monitor</span>
                    </button>
                    <button onClick={handleResetLocks} className="w-full p-4 bg-white/5 rounded-xl flex items-center gap-3 text-white/60 hover:text-red-300 hover:bg-red-500/10 transition-colors group">
                        <ShieldAlert size={16} className="group-hover:text-red-400"/>
                        <span className="text-xs font-mono">Reset Safety Locks</span>
                    </button>
                  </div>
              </section>
              
              <div className="pt-8 text-center">
                 <div className="text-[10px] text-white/20 font-mono">ZenB Kernel v6.0.0 (Sentient)</div>
              </div>
           </div>
        </div>
      </div>
